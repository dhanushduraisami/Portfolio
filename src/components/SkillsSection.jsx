...
export const SkillsSection = () => {
 ...
 ((skill) => activeCategory ==="all" || skill.category === activeCategory);
    return (
        <section id="skills" className="py-24 px-4 relative bg-secondery/30">
                ...
                <div className="flex flex-wrap justify-center gap-4 mb-12">
                    {categories.map((category,key) => (
                        ...
                        className={cn("px-5 py-2 rounded-full transition-colors duration-300 capitalize",
                            activeCategory === category ?"bg-primary text-primary-foreground":"bg-secondery/70 text-foreground hover:bd-secondary"
                        )}>
                            {category}
                        </button>
                    ))}
                        ...
                        className="bg-card p-6 rounded-lg shadow-xs card-hover">
                            ...
                            style={{width:skill.level + "%"}}/>
                             ...
                             <div className="text-right mt-1">
                                <spam className="text-sm text-muted-foreground">{skill.level}%</spam>
                            </div>
                        </div>
                    ))}
                </div>
